...
WsListener::WsListener(::net::io_context& ioc, const ::net::ip::tcp::endpoint& endpoint,
...
void WsListener::configureAcceptor() {
  ...
  // @see boost.org/doc/libs/1_61_0/doc/html/boost_asio/reference/basic_socket.html
  {
    ...
    /*if (enable_connection_aborted_) {
      acceptor_.set_option(::net::socket_base::enable_connection_aborted(true), ec);
      if (ec) {
        on_WsListener_fail(ec, "set_option");
        return;
      }
    }
    acceptor_.set_option(::net::socket_base::keep_alive(true), ec);
    if (ec) {
      on_WsListener_fail(ec, "set_option");
      return;
    }
    acceptor_.set_option(::net::socket_base::send_buffer_size(8192), ec);
    if (ec) {
      on_WsListener_fail(ec, "set_option");
      return;
    }
    acceptor_.set_option(::net::socket_base::receive_buffer_size(8192), ec);
    if (ec) {
      on_WsListener_fail(ec, "set_option");
      return;
    }*/
  ...
  if (max_listen_connections_ < 0) {
    ...
    max_listen_connections_ = ::net::socket_base::max_listen_connections;
  } else {
...
void WsListener::do_accept() {
  ...
  acceptor_.async_accept(
      socket_, ::net::bind_executor(strand_, std::bind(&WsListener::on_accept, shared_from_this(),
                                                       std::placeholders::_1)));
}
...
void WsListener::stop() {
   ...
   * Calls to cancel() will always fail with ::net::error::operation_not_supported when run on
   ...
   **/
  ::net::post(socket_.get_executor(), ::net::bind_executor(strand_, [&]() {
                try {
                  beast::error_code ec;
...
void WsListener::on_accept(beast::error_code ec) {
  ...
  if (ec) {
    ...
    const bool canCreateSessionsByRequest =
        mode_->_value == WS_LISTEN_MODE::SERVER || mode_->_value == WS_LISTEN_MODE::BOTH;

    const auto connectionsCount = nm_->getWS()->getSessionsCount();
    ...
    if (connectionsCount >= (max_listen_connections_ - 1) ||
      ...
      auto newWsSession = std::make_unique<WsSession>(std::move(socket_), nm_, newSessId);
